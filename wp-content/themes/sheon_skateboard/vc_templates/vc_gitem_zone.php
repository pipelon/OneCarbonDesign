...
if ('yes' === $featured_image) {
    ...
    $image = '<img src="{{ post_image_url' . ( false !== $background_image_css_editor ? ':' . rawurlencode($background_image_css_editor) . '' : ':' ) . ':' . $img_size . ' }}" class="" alt="{{ post_image_alt }}">';
} elseif (false !== $background_image_css_editor) {
    $image = '<img src="' . esc_url($background_image_css_editor) . '" class="" alt="{{ post_image_alt }}">';
}
if (strlen($link) > 0 && 'none' !== $link) {
    ...
    if ('custom' === $link && !empty($url)) {
        ...
        $image_block = '<a href="' . esc_url($link_s['url']) . '" title="' . esc_attr($link_s['title']) . '" target="' . esc_attr(trim($link_s['target'])) . '" class="vc_gitem-link vc-zone-link"' . $rel . '>' . $image . '</a>';
    } elseif ('post_link' === $link) {
        $image_block = '<a href="{{ post_link_url }}" title="{{ post_title }}" class="vc_gitem-link vc-zone-link">' . $image . '</a>';
    } elseif ('post_author' === $link) {
        $image_block = '<a href="{{ post_author_href }}" title="{{ post_author }}" class="vc_gitem-link vc-zone-link">' . $image . '</a>';
    } elseif ('image' === $link) {
        $image_block = '<a href="{{ post_image_url }}" title="{{ post_title }}" class="vc_gitem-link vc-zone-link">' . $image . '</a>';
    } elseif ('image_lightbox' === $link) {
        if (!isset($this->prettyphoto_rel)) {
            $this->prettyphoto_rel = ' data-rel="prettyPhoto[rel-' . get_the_ID() . '-' . wp_rand() . ']"';
        }
        $image_block .= '<a href="{{ post_image_url }}" title="{{ post_title }}" ' . $this->prettyphoto_rel . ' data-vc-gitem-zone="prettyphotoLink" class="vc_gitem-link prettyphoto vc-zone-link vc-prettyphoto-link">' . $image . '</a>';
    }
