...
function tmpmela_set_default_options()
{
    ...
    add_option("tmpmela_logo_image_alt", 'sheon'); // set logo image alt
    add_option("tmpmela_mob_logo_image", ""); // set logo image
    add_option("tmpmela_mob_logo_image_alt", 'sheon'); // set logo image alt
    add_option("tmpmela_showsite_description", 'no'); // yes/no, control panel
    add_option("tmpmela_site_icon", get_template_directory_uri() . "/images/megnor/favicon.png"); // set favicon icon
    ...
    add_option("tmpmela_bodyfont_color", "8c8c8c"); // body font color
    ...
    add_option("tmpmela_button_color", "f22b2b"); // button color
    ...
    add_option("tmpmela_header_back_repeat", "repeat"); // header background repeat
    ...
    add_option("tmpmela_header_topbkg_color", "F6F6F8"); // header top background color
    add_option("tmpmela_header_topbkg_opacity", "1"); // header top background opacity
    add_option("tmpmela_header_bottombkg_color", "ffffff"); // header bottom background color
    add_option("tmpmela_header_bottombkg_opacity", "1"); // header bottom background opacity
    add_option("tmpmela_header_toptext_color", "8A8A8A"); // header top text color
    add_option("tmpmela_header_toptexthover_color", "f22b2b"); // header top text hover color
    /*  Navigation Menu Settings  */
    add_option("tmpmela_menu_activelink_color", "F22B2B"); // Navigation menu active link color
    add_option("tmpmela_menu_activelinkhover_color", "000000"); // Navigation menu active link hover color
    add_option("tmpmela_menu_parentlink_color", "444444"); // Navigation menu parent link color
    add_option("tmpmela_menu_parentlinkhover_color", "f22b2b"); // Navigation menu parent link hover color
    add_option("tmpmela_menu_childlink_color", "000000"); // Navigation menu child link color
    add_option("tmpmela_menu_subchildlink_color", "444444"); // Navigation menu sub-child link color
    add_option("tmpmela_menu_childlinkhover_color", "f22b2b"); // Navigation menu sub-child link hover color
    /* Contact Detail setting*/
    add_option("tmpmela_show_contact", "yes"); // show contact detail
    add_option("tmpmela_contact_text_color", "ffffff"); // Top menu text color
    add_option("tmpmela_contact_number", "1.820.3345.33"); // Contact Number
    add_option("tmpmela_contact_email", "Sheon@Domain.com"); // Contact Email
    /* offer Text setting*/
    add_option("tmpmela_show_offer", "yes"); // show offer text
    add_option("tmpmela_offer_text", "Free shipping for all order over $1000"); // offer text
    /*  Content Settings  */
    ...
    add_option("tmpmela_link_color", "8A8A8A"); // link color
    add_option("tmpmela_hoverlink_color", "f22b2b"); // link hover color
    /*  Footer Settings  */
    add_option("tmpmela_footer_bkg_color", "070706"); // footer background color
    ...
    add_option("tmpmela_footerlink_color", "9d9d9d"); // footer link text color
    add_option("tmpmela_footerhoverlink_color", "f22b2b"); // footer link hover text color
...
/*for single product ptoduct thumbnail size*/
...
function tmpmela_get_logo()
{
    ...
    if (trim(get_option('tmpmela_logo_image')) != '') {
        echo '<img alt="' . esc_attr(get_option('tmpmela_logo_image_alt')) . '" src="' . get_option('tmpmela_logo_image') . '" />';
    } else {
...
function tmpmela_contact_detail()
{
    echo '<span class="contact-no"><span class="contact-number">' . esc_attr(stripslashes(get_option('tmpmela_contact_number'))) . '</span></span><span class="contact-ema"><span class="contact-email">' . esc_attr(stripslashes(get_option('tmpmela_contact_email'))) . '</span></span>';
}
...
{
    return ' <a class="read-more-link" href="' . esc_url(get_permalink()) . '">' . esc_html__('Read More', 'sheon') . '</a>';
}
/**
...
function tmpmela_excerpt($limit)
{
    ...
    if (count($excerpt) >= $limit) {
        array_pop($excerpt);
        $excerpt = implode(" ", $excerpt) . '&nbsp;.<div class="read-more"><a class="read-more-link" href="' . esc_url(get_permalink()) . '">' . esc_html__('Read More', 'sheon') . '</a></div>';
    } else {
...
function tmpmela_load_fonts()
{
    ...
    */
    $Poppins = _x('on', 'Poppins font: on or off', 'sheon');
    if ('off' !== $Poppins) {
        $font_families = array();
        if ('off' !== $Poppins) {
...
if (!function_exists('tmpmela_load_scripts')) :
    function tmpmela_load_scripts()
    {
        ...
        wp_enqueue_script('sticky-kit', get_template_directory_uri() . '/js/megnor/sticky-kit.js', array(), '', false);
        ?>
        <!--[if lt IE 9]>
...
function tmpmela_search_form($form)
{
    ...
    <div><label class="screen-reader-text" for="s">' . esc_attr('Search for:', 'sheon') . '</label>
    <input class="search-field" type="text" placeholder="' . esc_attr__('Search', 'sheon') . '" value="' . get_search_query() . '" name="s" id="s" />
    <input class="search-submit" type="submit" id="searchsubmit" value="' . esc_attr('Go', 'sheon') . '" />
    </div>
    </form>';
    return $form;
}
        ...
        switch ($comment->comment_type) :
            ...
            case 'trackback' :
                ...
                <li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
                    <p>
                        <?php esc_html_e('Pingback:', 'sheon'); ?>
                        <?php comment_author_link(); ?>
                        <?php edit_comment_link(esc_html__('(Edit)', 'sheon'), '<span class="edit-link">', '</span>'); ?>
                    </p>
                </li>
                <?php
                break;
                ...
                <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
                    <div id="comment-<?php comment_ID(); ?>" class="comment-body">
                        ...
                        <div class="author-content">
                            ...
                            <?php edit_comment_link(esc_html__('Edit', 'sheon'), ' '); ?>
                            <div class="clearfix"></div>
                            <abbr class="published"
                                  title="<?php echo get_the_title() ?>"><?php printf(esc_html__('%1$s at %2$s', 'sheon'), get_comment_date(), get_comment_time()); ?></abbr>
                        </div>
                        ...
                        <?php if ($comment->comment_approved == '0') : ?>
                            <em class="comment-awaiting-moderation">
                                <?php esc_html_e('Your comment is awaiting moderation.', 'sheon'); ?>
                            </em> <br/>
                        <?php endif; ?>
                    </div>
                    <!-- #comment-##  -->
...
if (!function_exists('tmpmela_sticky_post')) :
    function tmpmela_sticky_post()
    {
        if (is_sticky() && is_home() && !is_paged())
            echo '<div class="meta-inner"><span class="sticky-post">' . esc_html__('Sticky', 'sheon') . '</span></div>';
    }
endif;
if (!function_exists('tmpmela_categories_links')) :
    function tmpmela_categories_links()
    {
        ...
        $categories_list = get_the_category_list(esc_html__(', ', 'sheon'));
        if ($categories_list) {
...
if (!function_exists('tmpmela_tags_links')) :
    ...
    {
        // Translators: used between list items, there is a space after the comma.
        $tag_list = get_the_tag_list('', esc_html__(', ', 'sheon'));
        if ($tag_list) {
...
if (!function_exists('tmpmela_author_link')) :
    function tmpmela_author_link()
    {
        ...
        if ('post' == get_post_type()) {
            printf('<div class="meta-inner"><span class="author vcard"><i class="fa fa-pencil-square-o"></i><a class="url fn n" href="%1$s" title="%2$s" rel="author">' . esc_html__("by ", 'sheon') . '%3$s</a></span></div>',
                esc_url(get_author_posts_url(get_the_author_meta('ID'))),
                esc_attr(sprintf(esc_html__('View all posts by %s', 'sheon'), get_the_author())),
                get_the_author()
            );
        }
    }
...
if (!function_exists('tmpmela_comments_link')) :
    function tmpmela_comments_link()
    {
        //comments open
        if (comments_open()) :
            echo '<div class="meta-inner"><span class="comments-link"><i class="fa fa-comments"></i>';
            comments_popup_link(esc_html__('Leave a Comment', 'sheon'), esc_html__('1 Comment', 'sheon'), esc_html__('% Comments', 'sheon'));
            echo '</span></div>';
        endif;
    }
endif;
if (!function_exists('tmpmela_entry_date')) :
    ...
    function tmpmela_entry_date($echo = true)
    {
        if (has_post_format(array('chat', 'status')))
            $format_prefix = _x('%1$s on %2$s', '1: post format name. 2: date', 'sheon');
        else
            $format_prefix = '%2$s';
...
<time class="entry-date" datetime="%3$s">%4$s</time></a></span></div>',
            ...
            esc_attr(sprintf(esc_html__('Permalink to %s', 'sheon'), the_title_attribute('echo=0'))),
...
if (!function_exists('tmpmela_posts_short_description')) :
    function tmpmela_posts_short_description()
    {
        ...
        if (empty($tmpmela_posts_short_description))
            $tmpmela_posts_short_description = tmpmela_excerpt(20);
    ...
    function tmpmela_shortcode_paging_nav()
    {
        ...
        if ($GLOBALS['wp_query']->max_num_pages > 1) {
            ...
            $links = paginate_links(array(
                ...
                'prev_text' => wp_kses(__('<i class="fa fa-angle-left"></i>', 'sheon'), tmpmela_allowed_html()),
                'next_text' => wp_kses(__('<i class="fa fa-angle-right"></i>', 'sheon'), tmpmela_allowed_html()),
            ));
            if ($links) :
...
if (!function_exists('tmpmela_out_of_stock')) :
    function tmpmela_out_of_stock()
    {
        ...
        if (!$product->is_in_stock()) {
            echo '<div class="soldout_wrapper"><span class="soldout">' . esc_html__('Sold Out', 'sheon') . '</span></div>';
        }
    }
endif;
...
{
    if ($rating > 0) {
        $title = sprintf(__('Rated %s out of 5', 'sheon'), $rating);
    } else {
        $title = 'Not yet rated';
        $rating = 0;
    }
    $rating_html = '<div class="star-rating" title="' . $title . '">';
    $rating_html .= '<span style="width:' . (($rating / 5) * 100) . '%"><strong class="rating">' . $rating . '</strong> ' . esc_attr__('out of 5', 'sheon') . '</span>';
    $rating_html .= '</div>';
    return $rating_html;
}
...
function tmpmela_update_comment_fields($fields)
{
    ...
    $label = $req ? '*' : ' ' . __('(optional)', 'sheon');
    $aria_req = $req ? "aria-required='true'" : '';
    $fields['author'] =
        '<p class="comment-form-author comment-block">
<input id="author" name="author" type="text" placeholder="' . esc_attr__("Name*", "sheon") . '" value="' . esc_attr($commenter['comment_author']) .
        '" size="30" ' . $aria_req . ' />
</p>';
    $fields['email'] =
        '<p class="comment-form-email comment-block">
<input id="email" name="email" type="email" placeholder="' . esc_attr__("E-mail*", "sheon") . '" value="' . esc_attr($commenter['comment_author_email']) .
        '" size="30" ' . $aria_req . ' />
</p>';
    $fields['url'] =
        '<p class="comment-form-url comment-block">
<input id="url" name="url" type="url"  placeholder="' . esc_attr__("Website", "sheon") . '" value="' . esc_attr($commenter['comment_author_url']) .
        '" size="30" />
</p>';
    return $fields;
}
