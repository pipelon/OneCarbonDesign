    ...
    function tmpmela_paging_nav()
    {
        ...
        if ($links) :
            ...
            <nav class="navigation paging-navigation" role="navigation">
                <h3 class="screen-reader-text"><?php esc_html_e('Posts navigation', 'sheon'); ?></h3>
...
if (!function_exists('tmpmela_post_nav')) :
    ...
    { ?>
        ...
        ?>
        <div id="cooler-nav" class="navigation post-navigation">
            <?php
            if (is_attachment()) :
                previous_post_link('%link', wp_kses(__('<span class="meta-nav">Published In</span>%title', 'sheon'), tmpmela_allowed_html()));
            else :
                if ($prevPost) {
                    ...
                    previous_post_link('%link', wp_kses(__('<i class="fa fa-arrow-left"></i> &nbsp;%title', 'sheon'), tmpmela_allowed_html()));
                    echo '</div>';
                }
                if ($nextPost) {
                    echo '<div class="nav-box next">';
                    $nextthumbnail = get_the_post_thumbnail($nextPost->ID, array(50, 50));
                    echo wp_kses_post($nextthumbnail);
                    next_post_link('%link', wp_kses(__('%title &nbsp;<i class="fa fa-arrow-right"></i>', 'sheon'), tmpmela_allowed_html()));
                    echo '</div>';
                }
            endif;
            ?>
...
if (!function_exists('tmpmela_posted_on')) :
    ...
    function tmpmela_posted_on()
    {
        if (is_sticky() && is_home() && !is_paged()) {
            echo '<span class="featured-post">' . esc_html__('Sticky', 'sheon') . '</span>';
        }
