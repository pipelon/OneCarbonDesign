...
function tmpmela_setup() {
	...
	* replace to change 'sheon' to the name of your theme in all
	* template files.
	*/
	load_theme_textdomain( 'sheon', get_template_directory() . '/languages' );
	/*
	...
	register_nav_menus( array(
		 'primary'   => esc_html__( 'TM Header Navigation', 'sheon' ),
		 'header-menu'   => esc_html__( 'TM Header Top Links', 'sheon' ),
		 'account-menu'   => esc_html__( 'TM Header Account Links', 'sheon' ),
		 'footer-menu'   => esc_html__( 'TM Footer Navigation', 'sheon' ),
	) );
	/*
	...
	set_post_thumbnail_size( 630, 296, true );
...
function tmpmela_widgets_init() {
	register_sidebar( array(
		'name' => esc_html__( 'Main Sidebar', 'sheon' ),
		'id' => 'sidebar-1',
		'description' => esc_html__( 'Appears on posts and pages except the optional Front Page template, which has its own widgets', 'sheon' ),
...
function tmpmela_fonts_url() {
	 ...
	 */
	$source_sans_pro = _x( 'on', 'Source Sans Pro font: on or off', 'sheon' );
	/* Translators: If there are characters in your language that are not
	 * supported by Bitter, translate this to 'off'. Do not translate into your
	 * own language.
	 */
	$bitter = _x( 'on', 'Bitter font: on or off', 'sheon' );
...
<div class="contributor">
  <div class="contributor-info">
    ...
    <div class="contributor-summary">
      ...
      <a class="contributor-posts-link" href="<?php echo esc_url( get_author_posts_url( $contributor_id ) ); ?>"> <?php printf( _n( '%d Article', '%d Articles', $post_count, 'sheon' ), $post_count ); ?> </a> </div>
    <!-- .contributor-summary -->
...
function tmpmela_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'sheon' ), max( $paged, $page ) );
	}
	return $title;
}
...
<div class="related_posts_outer"><div class="shortcode-title center fancy"><h1 class="normal-title"><?php echo esc_attr_e( 'Related Posts', 'sheon' ) ?></h1></div>
...
function tmpmela_sampledata_import_files() {
    return array(
        array(
            ...
            'local_import_file'            => trailingslashit( get_template_directory() ) . '/demo-content/default/sheon.wordpress.xml',
            'local_import_customizer_file' => trailingslashit( get_template_directory() ) . '/demo-content/default/sheon_customizer_export.dat',
			'local_import_widget_file'     => trailingslashit( get_template_directory() ) . '/demo-content/default/sheon_widgets_settings.wie',
...
function tmpmela_sampledata_after_import() {
        //Set Menu
