...
function shortcode_blog_posts_container($atts, $content = null, $code) {
	...
	while ( $blog_array->have_posts() ) : $blog_array->the_post();
		...
		if ( has_post_thumbnail() && ! post_password_required() ) :	
			...
			$image = get_template_directory_uri()."/images/megnor/placeholder.jpg";	
		endif;
		...
		if($style == '1'):
				$output .= '<div class="item container '.$class.'  style-'.$style.'">';
					$output .= '<div class="container-inner">';
						...
						$output .= '<div class="meta-inner"><span class="date"><a href="'.get_permalink().'">
<span class="entry-date">'.$post_day.' '.$post_month.' '.$post_year.'</span></a></span></div>';	
						$output .= '</div>';
						...
						$output .= '<div class="post-description">'.tmpmela_excerpt(10).'</div>';
			$output .= '</div></div></div>';
		endif;
		$i++;
	endwhile;
	...
	$output .= '<div class="no-result">'.esc_html__('No results found...', 'sheon').'</div>';
	endif;
	return $output;
}
...
function shortcode_woo_category_slider($atts, $content = null, $code) {
	...
	$output .= '<div id="categorytab">';
		...
		$term_thumbnai_id = '';
		$term_image = '';			
		$output .= '<ul class="resp-tabs-list '.$type.'">';
...
function shortcode_cms_block($atts, $content = null, $code) {
	...
	$cmsimg = '';	
		...
		$output .='<div class="cms-banner-item '.$classname.'"><div class="cms-banner-inner">'.$cmsimg.'<span class="static-wrapper">'.$cmstext1.''.$cmstext2.''.$cmstext3.''.$link_text.'</span></div></div>';
...
function shortcode_portfolio_slider_container($atts, $content = null, $code) {
				...
				$output .= '<div class="product-block single-portfolio '.$class.' '.$layout.'">';
					$output .= '<div class="portfolio-image">';
							...
							$output .= '<div class="block_hover">';
								...
								$output .= '<a href="'.get_permalink().'" title="'.esc_html__('Click to view read more', 'sheon').'" class="icon"><i class="fa fa-link"></i></a>';							
								$output .= '</div>';
							$output .= '</div>';
					$output .= '</div>';
					...
					$output .= '<div class="read-more"><a href="'.get_permalink().'" title="'.get_the_title().'">'.esc_html__('read more', 'sheon').'</a></div>';				
				$output .= '</div>';
				$output .= '</div>';
				$i++;
			endwhile;
	...
	$output .= '<div class="no-result">'.esc_html__(''.esc_html__('No results found...', 'sheon').'', 'sheon').'</div>';
	endif;
	return $output;
}
...
function shortcode_woo_products_container($atts, $content = null, $code) {
	...
	if($type == "grid") {  
		...
		$output .=	'<div class="loadgridlist-wrapper"><button class="woocount loadgridlist">'.esc_html__('View More Products', 'sheon').'</button></div>';		
	}
...
function shortcode_title($atts, $content = null) {
   ...
   'classname' =>''
	...
	$output = '';	
	$output .= '<div class="shortcode-title '.$style.' '.$classname.'">';
	if(!empty($color))	
	...
	foreach($testimonial_array as $post) : setup_postdata($post);	
		...
		$output .= '<div class="product-block">';
			$output .= '<div class="custom-testimonial-inner">';	
				...
				if($style == '1'):	
					$output .= '<div class="testimonial-top"><blockquote><q>'.substr($contents, 0, 120).'</q></blockquote></div>';
				endif;		
				$output .= '</div>';
...
return __( 'Missing Attachment', 'sheon' );
...
function shortcode_newsletter_text($atts, $content = null){
  extract(shortcode_atts(array(
    'text1' => 'Join the Community to be Updated Firstly ?',
    'text2' => 'Signup for our newletter and promotion',
	'classname'=>'',
   ), $atts));
  $output = '';  
  $output .= '<div class="container '.$classname.'">';
  $output .= '<div class="newslettercontainer">';
  $output .= '<div class="newsletter-left-content">';
  if(!empty($text1))
     $output .= '<div class="text1">'.$text1.'</div>';
  if(!empty($text2))
    $output .= '<div class="text2">'.$text2.'</div>';
  $output .= '</div>';	
  $output .= '<div class="newsletter-right-content">';  
  $output .=  do_shortcode($content);
  $output .= '</div></div></div>';
  return $output;
}
